    ...
    <section>
      ...
      <div className="px-4 flex flex-wrap gap-2 items-center mb-3">
        <div className="flex items-center">
          ...
          >
            {periodos.map((p) => {
              // Remove " - Unidade X" do display se existir, conforme solicitado
      ...
      {commentsItem && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center" onClick={() => setCommentsItem(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-lg shadow-lg w-full max-w-xl p-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-2">{t('supervision_comments_title') || 'Comentários do supervisor'}</h3>
            <div className="space-y-2 max-h-[50vh] overflow-y-auto">
              {commentsList.map((c) => (
                <div key={c.id} className="p-2 rounded border border-slate-200 dark:border-slate-800">
                  <div className="text-xs text-slate-500">
                    {new Date(c.data_comentario).toLocaleString('pt-BR')} • {t('supervision_comments_supervisor_label') || 'Supervisor:'} {c.supervisor_id}
                  </div>
                  <div className="text-sm mb-1">{c.comentario}</div>
                  <div className="text-xs">
                    {t('supervision_comments_status') || 'Status:'} {c.status} • {t('supervision_comments_type') || 'Tipo:'} {c.tipo}
                  </div>
                  {c.resposta ? (
                    <div className="mt-1 pl-2 border-l-2 border-slate-300">
                      <div className="text-xs text-slate-500">
                        {t('supervision_comments_reply_header', { date: new Date(c.data_resposta).toLocaleString('pt-BR'), user: c.respondido_por }) || `Resposta (${new Date(c.data_resposta).toLocaleString('pt-BR')} • ${c.respondido_por})`}
                      </div>
                    ...
                    !periodoEncerrado && (
                      <div className="mt-2">
                        <label className="block text-sm mb-1">{t('supervision_comments_reply_label') || 'Responder'}</label>
                        <textarea
                          className="w-full rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-2"
                          value={replyText}
                          onChange={(e) => setReplyText(e.target.value)}
                          placeholder={t('supervision_comments_reply_placeholder') || 'Escreva sua resposta'}
                        />
                        <div className="mt-2 flex gap-2">
                          <button className="px-3 py-1 rounded border" onClick={() => setCommentsItem(null)}>
                            {t('supervision_comments_close') || 'Fechar'}
                          </button>
                          <button className="px-3 py-1 rounded bg-blue-600 text-white" onClick={() => sendReply(c)}>
                            {t('supervision_comments_send') || 'Responder'}
                          </button>
                        </div>
                      </div>
                    )
              ...
              {commentsList.length === 0 && (
                <div className="text-xs text-slate-500">{t('supervision_comments_empty') || 'Sem comentários.'}</div>
              )}
            </div>
            {periodoEncerrado && (
              <div className="mt-2 text-xs text-amber-700">
                ⚠️ {t('supervision_comments_period_closed') || 'Período encerrado: respostas bloqueadas.'}
              </div>
            )}
          </div>
        </div>
