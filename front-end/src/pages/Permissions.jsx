...
export default function PermissionsPage() {
  ...
  const seedDefaultTransactions = React.useCallback(async () => {
    try {
      ...
      const routeNames = {
        ...
        '/permissions/logs': 'Central de Logs',
      };
      ...
      if (anyChange) {
        ...
        toast.success(t('permissions_routes_updated') || 'Rotas padrão atualizadas');
      } else {
        toast(t('permissions_routes_already_updated') || 'Rotas já estão atualizadas');
      }
    } catch (err) {
      console.error(err);
      toast.error(t('permissions_routes_manage_error') || 'Erro ao gerenciar rotas padrão');
    } finally {
      setLoading(false);
    }
  }, [transactions, t]);
  ...
  const onEdit = async (row) => {
    try {
      ...
      console.error(err);
      toast.error(t('permissions_open_group_error') || 'Erro ao abrir grupo');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSave = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(t('permissions_save_error') || 'Erro ao salvar');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onDelete = async (id) => {
    try {
      ...
      console.error(err);
      toast.error(t('permissions_delete_error') || 'Erro ao excluir');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onAddCompany = async () => {
    ...
    try {
      ...
      toast.success(t('permissions_company_linked') || 'Empresa vinculada');
    } catch (err) {
      console.error(err);
      toast.error(t('permissions_company_link_error') || 'Erro ao vincular empresa');
    }
  };
  const onRemoveCompany = async (empresaId) => {
    ...
    try {
      ...
      toast.success(t('permissions_company_unlinked') || 'Empresa desvinculada');
    } catch (err) {
      console.error(err);
      toast.error(t('permissions_remove_link_error') || 'Erro ao remover vínculo');
    }
  };
  ...
  const onAddTransaction = async () => {
    ...
    try {
      ...
      toast.success(t('permissions_transaction_linked') || 'Transação vinculada');
    } catch (err) {
      console.error(err);
      toast.error(t('permissions_transaction_link_error') || 'Erro ao vincular transação');
    }
  };
  const onRemoveTransaction = async (transacaoId) => {
    ...
    try {
      ...
      toast.success(t('permissions_transaction_unlinked') || 'Transação desvinculada');
    } catch (err) {
      console.error(err);
      toast.error(t('permissions_remove_link_error') || 'Erro ao remover vínculo');
    }
  ...
  const onAddUser = async () => {
    ...
    try {
      ...
      toast.success(t('permissions_user_linked') || 'Usuário vinculado');
    } catch (err) {
      console.error(err);
      toast.error(t('permissions_user_link_error') || 'Erro ao vincular usuário');
    }
  };
  const onRemoveUser = async (usuarioId) => {
    ...
    try {
      ...
      toast.success(t('permissions_user_unlinked') || 'Usuário desvinculado');
    } catch (err) {
      console.error(err);
      toast.error(t('permissions_remove_link_error') || 'Erro ao remover vínculo');
    }
  ...
  const onClone = async () => {
    if (!editingId || !cloneName.trim()) {
      toast.error(t('permissions_clone_name_required') || 'Informe o novo nome');
      return;
    }
    try {
      setLoading(true);
      const novo = await clonePermissionGroup(editingId, { novo_nome: cloneName.trim(), descricao: form.descricao || '' });
      toast.success(t('permissions_group_cloned') || 'Grupo clonado');
      await load();
      ...
      console.error(err);
      toast.error(t('permissions_clone_error') || 'Erro ao clonar grupo');
    } finally {
      setLoading(false);
    }
  };
