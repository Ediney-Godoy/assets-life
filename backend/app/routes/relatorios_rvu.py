...
def _log_relatorio(
    ...
    empresa_id: int | None,
    tipo_arquivo: str,
    parametros_usados: str,
    caminho_arquivo: str | None,
):
    try:
        with db.connection() as conn:
            cols = [c['name'] for c in sa.inspect(conn).get_columns('relatorios_rvu_logs')]
            fields = []
            values = {}
            if 'usuario_id' in cols:
                fields.append('usuario_id')
                values['usuario_id'] = usuario_id
            if 'empresa_id' in cols:
                fields.append('empresa_id')
                values['empresa_id'] = empresa_id
            if 'tipo_arquivo' in cols:
                fields.append('tipo_arquivo')
                values['tipo_arquivo'] = tipo_arquivo
            if 'parametros_usados' in cols:
                fields.append('parametros_usados')
                values['parametros_usados'] = parametros_usados
            if 'caminho_arquivo' in cols:
                fields.append('caminho_arquivo')
                values['caminho_arquivo'] = caminho_arquivo
            if 'data_emissao' in cols:
                fields.append('data_emissao')
                values['data_emissao'] = datetime.utcnow()
            if not fields:
                return
            placeholders = ", ".join(f":{f}" for f in fields)
            sql = f"INSERT INTO relatorios_rvu_logs ({', '.join(fields)}) VALUES ({placeholders})"
            conn.execute(sa.text(sql), values)
            conn.commit()
    except Exception:
        pass
...
def list_log(current_user: UsuarioModel = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        with db.connection() as conn:
            cols = [c['name'] for c in sa.inspect(conn).get_columns('relatorios_rvu_logs')]
            required = ['data_emissao', 'usuario_id', 'empresa_id', 'tipo_arquivo', 'parametros_usados', 'caminho_arquivo']
            missing = [c for c in required if c not in cols]
            if missing:
                return []

            sql = "SELECT id, data_emissao, usuario_id, empresa_id, tipo_arquivo, parametros_usados, caminho_arquivo FROM relatorios_rvu_logs ORDER BY data_emissao DESC"
